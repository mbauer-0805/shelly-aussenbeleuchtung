...
var CONFIG = {
  ...
  schedules: {
    ...
    weekday: { 
      morning: { 
        enabled: true,     // Morgenschaltung aktiv
        onH: 6,            // Einschaltzeit Stunde (z.B. 6 Uhr)
        onM: 30,           // Einschaltzeit Minute (z.B. 30 Minuten)
        offOffsetMin: 10   // Ausschaltzeit: Minuten nach Referenz (z.B. 10 Minuten nach Sonnenaufgang)
      },
      evening: { 
        enabled: true,             // Abendschaltung aktiv
        onOffsetBeforeRefMin: 10,  // Einschaltzeit: Minuten vor Referenz (z.B. 10 Minuten vor Sonnenuntergang)
        offH: 22,                  // Ausschaltzeit Stunde (z.B. 22 Uhr)
        offM: 0                    // Ausschaltzeit Minute (z.B. 0 Minuten)
      }
    },
    // Wochenend-Zeitplan
    weekend: { 
...
(function validateConfig() {
  ...
  ["weekday", "weekend"].forEach(function(type) {
    var morning = s[type].morning;
    var evening = s[type].evening;
    var hm = Utils.normHM(morning.onH, morning.onM);
...
// RPC queue manager
var RPC = (function() {
  var queue = [], index = 0, busy = false;
  function enqueue(method, params, cb, timeout) {
    queue.push({ method: method, params: params || {}, cb: cb, timeout: timeout || CONFIG.rpcTimeout });
    pump();
  }
  function pump() {
    if (busy) return;
    ...
    var job = queue[index++];
    var timedOut = false;
    var timer = Timer.set(job.timeout, false, function() {
      if (timedOut) return;
      timedOut = true;
...
var ScheduleManager = (function() {
  function ensureSwitchAt(h, m, on, existingMap, sigIndex) {
    ...
    var sig = jobSigSwitch(h, m, on);
    var curId = sigIndex[sig];
    if (curId) {
      var upd = { id: curId, timespec: desired.timespec, enable: true, calls: desired.calls };
...
function updateSchedulesForToday() {
  ...
  TwilightAPI.fetch(now, function(results) {
    ...
    var morningRef = results[morningKey], eveningRef = results[eveningKey];
    ...
    var morningDate = new Date(morningRef), eveningDate = new Date(eveningRef);
...
function applySchedulesDiff(schedule, refMornSec, refEveSec, morningDate, eveningDate, existingMap, sigIndex) {
  var morning = schedule.morning, evening = schedule.evening;
  ...
  if (morningAllowed) {
    ...
    var offH = morningDate.getHours(), offM = morningDate.getMinutes() + morning.offOffsetMin;
